    ...
    while change >= tolerance:
        ...
        for row in range(len(path)):
            for col in range(len(path[0])):
                ...
                newpath[row][col] += (weight_data * (path[row][col] - newpath[row][col]) 
                                    + weight_smooth * (newpath[(row + 1) % len(path)][col] + newpath[row - 1][col] - 2 * newpath[row][col]))
                
                change += abs(old - newpath[row][col])
    return newpath
