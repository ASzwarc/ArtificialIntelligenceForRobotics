...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        x1, y1 = spath[index]
        R = [estimate[0] - x1, estimate[1] - y1]
        x2, y2 = spath[index + 1]
        u = (R[0] * (x2 - x1) + R[1] * (y2 - y1)) / sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)
        cte = (R[1] * (x2 - x1) - R[0] * (y2 - y1)) / sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)
        ...
        if printflag:
            print("{} {} {}".format(myrobot, cte, index))
        ...
        for i in range(len(params)):
            ...
            if err < best_error:
                ...
                if err < best_error:
                    ...
                    dparams[i] *= 0.5
        n += 1
        print('Twiddle #{} {} -> {}'.format(n, params, best_error))
    print(' ')
    return params
