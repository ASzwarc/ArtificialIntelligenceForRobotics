...
# single (x, y) point. This function will have to be called multiplecl
...
def estimate_next_pos(measurement, OTHER = None):
    ...
    #     point1 = OTHER[-1]
    #     point2 = OTHER[-2]
    #     velocity = distance_between(point1, point2)
    #     angle = atan2(point1[1] - point2[1], point1[0] - point2[0])
    #     if len(angles) > 1:
    #         diff_angles = []
    #         for i in range(1, len(angles)):
    #             diff_angles.append(angles[i] - angles[i - 1])
    #         angle_correction = sum(diff_angles) / len(diff_angles)
    #     else:
    #         angle_correction = sum(angles) / len(angles)

    #     xy_estimate = (point1[0] + velocity * cos(angle - angle_correction) - 0.2, point1[1] + velocity * sin(angle - angle_correction))
    # else:
    #     xy_estimate = OTHER[-1]
    if not OTHER:
        OTHER = []
...
def demo_grading(estimate_next_pos_fcn, target_bot, OTHER = None):
    ...
    while not localized and ctr <= 10: 
        ...
        print("CTR: {} GUESS: {} TRUE POSITION: {}".format(ctr, position_guess, true_position))
        plt.plot(true_position[0], true_position[1], 'bs', position_guess[0], position_guess[1], 'g^')
        if error <= distance_tolerance:
            print("You got it right! It took you {} steps to localize.".format(ctr))
            localized = True
        if ctr == 10:
...
def demo_grading_with_visualization(estimate_next_pos_fcn, target_bot, OTHER = None):
    localized = False
    distance_tolerance = 0.01 * target_bot.distance
    ctr = 0
    # if you haven't localized the target bot, make a guess about the next
...
#demo_grading_with_visualization(estimate_next_pos, test_target)
